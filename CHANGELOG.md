...
- Refactored `backend/vulture_whitelist.py` from bare-name references to a `Whitelist` class with a `__getattr__` method and a module-level `whitelist` instance. Names are now referenced as attributes (e.g., `whitelist.health`) instead of bare string expressions, fixing mypy `name-defined` errors and resolving lint issues with bare string expressions.
### Fixed
