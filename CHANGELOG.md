...
### Fixed
- `apply_choice_outcome` (in `backend/models/game_state.py`) no longer raises an uncaught `ValueError` on malformed/non-integer stat values. Each `stat:value` part is now parsed defensively by splitting on the FIRST colon only (`split(":", 1)`), so values containing extra colons (e.g. `credits:50:bonus`) are no longer silently truncated. The value is converted to `int` inside a `try`/`except`: if a recognized stat carries a non-integer value (e.g. `fuel:5x`, `fuel:2.5`), a warning is logged and that part is skipped (effect stays at the default of 0) while valid parts still apply.
