...
- `resolve_event` (in `backend/generation/events.py`) now applies the choice outcome BEFORE marking the event as `resolved`/`chosen`, making resolution atomic (the event is only marked resolved after the outcome is applied successfully).
