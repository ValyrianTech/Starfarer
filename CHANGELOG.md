...
- Race condition in per-game lock cleanup allowing concurrent access to the same game: both the periodic cleanup inside `_get_lock()` (every 100 accesses) and `_cleanup_stale_locks()` could remove a lock for a game no longer in `GAME_STORE` even while another thread was holding it (e.g., a game being loaded from the database inside `_get_state`). Both cleanup paths now check `not lock.locked()` before removing a lock, so a lock that is currently held by another thread is preserved. This prevents two threads from operating on the same game concurrently when one is mid-load.
