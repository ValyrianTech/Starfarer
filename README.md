...
`POST /api/game/new` returns a per-game `token` alongside `game_id` and `state`. The leaderboard endpoint (`GET /api/leaderboard`) returns an opaque `entry_id` and never exposes the raw `game_id` or `seed` — a deliberate breaking change from the original design. Per-game token enforcement is **ON by default** (secure baseline) so multi-user deployments are not vulnerable to IDOR: all endpoints that access a specific game — both mutating and read-only GET endpoints — require that token via the `X-Game-Token` header only (the `token` query parameter is no longer accepted). It can be disabled only by setting `STARFARER_ALLOW_NO_AUTH=1` (purely local, single-user play — **never** use it in a shared/networked deployment) or explicitly with `STARFARER_REQUIRE_GAME_TOKEN` set to a falsy value. The spectator SSE stream (`GET /api/spectate/{game_id}/stream`) authenticates via the `X-Game-Token` header or a short-lived single-use stream ticket (see Spectator Mode), and `GET /api/spectate/games` is disabled (HTTP 403) while enforcement is on. CORS credentials are automatically disabled when a wildcard origin is configured.
...
| `STARFARER_REQUIRE_GAME_TOKEN` | When explicitly set, controls per-game token enforcement — truthy (`1`, `true`, `yes`, `on`, case-insensitive) enables it, falsy (`0`, `no`, `false`, `off`) disables it. When unset, enforcement is **ON by default** (secure baseline). Requires the per-game token (supplied via the `X-Game-Token` header only; the `token` query parameter is no longer accepted) on all endpoints that access a specific game — mutating and read endpoints — plus spectator endpoints |
| `STARFARER_ALLOW_NO_AUTH` | Explicit opt-out to run without per-game token enforcement, for purely local, single-user play only. Truthy (`1`, `true`, `yes`, `on`, case-insensitive) disables enforcement. **Never** set this in a shared/networked deployment (default: off) |
