...
The response also includes a `token`. Per-game token enforcement is **ON by default** (secure baseline), so every endpoint that accesses a specific game — both the state-mutating endpoints (jump, scan, land, explore, atmospheric-scan, sub-surface-explore, event resolve, trade, bulk-sell, upgrade, distress, salvage, salvage/craft, faction mission accept, missions accept/complete, save, load, hints/dismiss) and the read-only GET endpoints (full game state, galaxy, system detail, log, log/paginated, discoveries, cargo, lore, codex, upgrades, nearby, factions, faction detail, missions, crossroads items, crossroads lore, crossroads messages) — requires the token, supplied via the `X-Game-Token: <token>` header only (the `token` query parameter is no longer accepted). You can disable enforcement only for purely local, single-user play by starting the server with `STARFARER_ALLOW_NO_AUTH=1` (**never** do this in a shared/networked deployment), or explicitly with `STARFARER_REQUIRE_GAME_TOKEN` set to a falsy value. A missing token returns HTTP 403 with detail "Game token required"; a wrong token returns HTTP 403 with detail "Invalid game token".
...
The full game state response (`GET /api/game/{id}`) includes a `hints` field — an array of active contextual hints that provide real-time survival tips. Hints are generated by evaluating current game state conditions and are designed to help new players avoid common pitfalls.

#### Hint Severity Levels

| Severity | Meaning |
|----------|---------|
| `critical` | Immediate action required — cannot be dismissed |
| `warning` | Important advisory — can be dismissed |
| `info` | General information — can be dismissed |
| `tip` | Helpful suggestion — can be dismissed |

#### Hint Definitions

| Hint ID | Severity | Trigger Condition |
|---------|----------|-------------------|
| `fuel_zero` | critical | Ship has 0 fuel — activates salvage/distress options |
| `fuel_critical` | warning | Fuel is critically low (below 5 units) |
| `fuel_low_no_station` | warning | Fuel is low and no trading station is in range |
| `hull_low` | warning | Hull integrity is below 30% of maximum |
| `morale_low` | warning | Crew morale is below 30 |
| `first_uncharted` | info | Player has not yet explored their first body |
| `cargo_full` | tip | Cargo hold is at or near maximum capacity |
| `first_crisis` | warning | First crisis-level event encountered |

A maximum of 2 hints are returned at a time, ordered by priority (critical > warning > info > tip). Critical hints (`fuel_zero`) cannot be dismissed.

#### Dismissing Hints

Non-critical hints can be dismissed for the remainder of the session:

```http
POST /api/game/{game_id}/hints/dismiss
Content-Type: application/json

{ "hint_id": "fuel_critical" }
```

Dismissed hints are tracked in the `dismissed_hints` set on the game state and persist across save/load.
...
| POST | `/api/game/{id}/hints/dismiss` | Dismiss a contextual hint for the session |
